...
namespace Malcha.Repository
{
    ...
    internal class ResultRepository
    {
        ...
        {
            var key = TrainingEpochDisplay.NormalizeModelName(name);
            return _results
                .Where(r => TrainingEpochDisplay.NormalizeModelName(r.Name)
        ...
        public async Task DeleteFromDatabaseAsync(string databasePath, TrainingResult target)
        {
            ...
            {
                var key = TrainingEpochDisplay.NormalizeModelName(target.Name);
                entries.RemoveAll(e =>
                    TrainingEpochDisplay.NormalizeModelName(e.Name ?? string.Empty)
                        .Equals(key, StringComparison.OrdinalIgnoreCase)
        ...
        {
            var key = TrainingEpochDisplay.NormalizeModelName(modelName);
            return !_results.Any(r =>
                TrainingEpochDisplay.NormalizeModelName(r.Name)
                    .Equals(key, StringComparison.OrdinalIgnoreCase));
        }
        ...
        private static TrainingResult MapEntry(DatabaseEntry entry, int index)
        {
            ...
            {
                Number = number, Name = entry.Name ?? string.Empty, Pilot = entry.Pilot ?? string.Empty,
