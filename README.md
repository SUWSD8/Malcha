...
* **1.데이터 선택:** 데이터 선택 버튼을 클릭하여 주행 .catalog 파일을 엽니다.

![과제1](img/2.png)

* **2.수동 정제 (선택 사항):** 선택구간 삭제 기능을 활용하여 불필요한 주행 구간이나 노이즈를 수동으로 편집합니다. (복구가 필요한 경우 되돌리기 사용)

![과제1](img/3.png)

* **3.필터 적용:** 필터 적용 버튼을 눌러 중복 프레임, 조향 스파이크, 범위를 초과하는 이상치를 자동으로 정제합니다. (보통 1회 실행으로 충분합니다.)

![과제1](img/9.png)

* **4.정제 데이터 연동 (학습 전 필수):** 정제된 카탈로그와 이미지를 WSL mycar/data 폴더로 동기화합니다.

### 주의: 이 과정을 거치지 않으면 WSL에 남아있는 예전 데이터로 학습이 진행됩니다.

* **5.학습 시작:** 학습 시작 버튼을 눌러 WSL에서 train.py를 실행합니다. (최초 실행 시 mycar 폴더 경로 지정이 필요합니다.)
